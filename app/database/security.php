...
class AuthDB 
{
    ...
    private PDOStatement $statementDestroySession;
    ...
    function __construct(private PDO $pdo)
    {
        ...
        $this->statementReadUser = $pdo->prepare('SELECT * FROM user WHERE id=:userid');
        ...
        $this->statementDestroySession = $pdo->prepare('DELETE FROM session WHERE id=:id');
    }
    ...
    function isLoggedin(): array | false
    {
            ...
            if ($sessionId && $signature) {
                ...
                if (hash_equals($hash, $signature)) {
                    ...
                    if ($session) {
                        $this->statementReadUser->bindValue(':userid', $session['userid']);
    ...
    {
        $this->statementDestroySession->bindValue(':id', $sessionId);
        $this->statementDestroySession->execute();
...
//         if ($session) {
//             $statementUser = $pdo->prepare('SELECT * FROM user WHERE id=:userid');
//             $statementUser->bindValue(':userid', $session['userid']);
